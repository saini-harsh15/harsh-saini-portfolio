...
const featuredProjects: Project[] = [
  ...
  {
    title: "Quirk – QR Code Generator",
    description:
        "A production-ready QR code generator built with Java Spring Boot, Thymeleaf, and ZXing. Supports PNG and SVG export, custom foreground/background colors, logo embedding, and a fully responsive UI with dark mode — backed by a layered service architecture with Bean Validation and global exception handling.",
    image: "/Quirk.png",
    technologies: ["Java", "Spring Boot", "Thymeleaf", "ZXing"],
    liveDemoUrl: "https://quirk-hvbc.onrender.com/",
    githubUrl: "https://github.com/saini-harsh15/Quirk-QR-Code-Generator",
  },
];
...
const FeaturedCard = ({ project, delay }: { project: Project; delay: number }) => (
    <div
        className="animate-fade-in-up rounded-2xl border border-white/20 bg-white/10 dark:bg-white/5 backdrop-blur-xl shadow-xl overflow-hidden flex flex-col h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:border-emerald-400/30"
    ...
    >
      ...
      <span className="inline-flex w-fit mb-4 px-3 py-1 rounded-full bg-gradient-to-r from-amber-500 to-emerald-500 text-white text-sm font-semibold">
        ...
        <div className="mt-auto">
          <ProjectLinks githubUrl={project.githubUrl} liveDemoUrl={project.liveDemoUrl} variant="default" />
        </div>
      </div>
    </div>
);
...
const ProjectCard = ({ project, delay }: { project: Project; delay: number }) => (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <p className="text-muted-foreground mb-4 leading-relaxed min-h-[110px]">{project.description}</p>
  ...
  return (
      <section id="projects" className="relative py-20 overflow-hidden bg-gradient-to-br from-zinc-100 to-stone-100 dark:from-zinc-950 dark:to-stone-900">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Featured projects — two large cards side by side */}
          <div className="grid lg:grid-cols-2 gap-8 mb-8">
            {featuredProjects.map((project, index) => (
                <FeaturedCard key={project.title} project={project} delay={100 + index * 100} />
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {otherProjects.map((project, index) => (
                <ProjectCard key={project.title} project={project} delay={300 + index * 100} />
            ))}
          </div>
        </div>
      </section>
